...
| sector | 業種トレンド＋業種内の相対強度（業種データがある時のみ寄与） | 0.15 |
...
## 業種（セクター）スコア（地合いを業種粒度で見る）

個別株の値動きは「市場全体＋業種要因＋個別要因」に分解できます。良いシグナルでも
**業種が逆風だと負けやすい**ため、合議スコアに `sector` コンポーネント（−1〜+1）を加え、
**業種が上昇トレンドか**＋**業種内で相対的に強いか（相対強度）**を確度に反映します。

データの取り方（**J-Quants 無料版の12週遅延を回避する設計**）:

- **業種の分類（コード→17/33業種）だけ J-Quants 無料版から取得**します。分類はほぼ静的
  なので遅延は無関係。米国株は yfinance の sector を使います。分類は `sectors` テーブルに
  キャッシュします。
- **業種のトレンド／相対強度は、スキャン時に取得する yfinance 価格から合成**します
  （構成銘柄の正規化リターンを等ウェイト平均した合成業種インデックス）。価格の追加取得は
  不要で、遅延の影響も受けません。
- 業種分類が未同期・構成銘柄が少ない業種は、`sector` 成分を出さず従来スコアへ無害に
  フォールバックします（重みは内部で正規化）。

### 業種分類の同期（初回・以後は月1回程度）

```bash
# .env に JQUANTS_REFRESH_TOKEN（または JQUANTS_MAILADDRESS/PASSWORD）を設定してから:
.venv/bin/python -m scripts.sync_sectors            # 日本株(J-Quants)＋米国株(yfinance)
.venv/bin/python -m scripts.sync_sectors --market JP
.venv/bin/python -m scripts.sync_sectors --dry-run  # 書き込まず取得件数を確認
```

cron 例（毎月1日 7:00 に分類を更新）:

```cron
0 7 1 * * /path/to/StockTrade/.venv/bin/python -m scripts.sync_sectors >> /path/to/StockTrade/cron.log 2>&1
```

挙動・重みは `config.py` の `SECTOR_CONFIG` と `SCORING_CONFIG["weights"]["sector"]` で調整
できます。バックテストでは `--no-sector` で業種スコアを無効化して **A/B 比較**できます。
...
.venv/bin/python -m backtest.runner --no-sector --save     # 業種スコア無効で比較（A/B）
...
- [x] 業種（セクター）トレンド＋相対強度を合議スコアに反映（業種分類は J-Quants 無料版/yfinance、トレンドは合成インデックス）

検討中:

- [ ] 機械学習による売買パラメータ（しきい値）の最適化 ※シグナル自体はテクニカルのまま（`docs/ROADMAP_PLAN.md` に構想）
- [ ] J-Quants APIで東証全銘柄を動的取得（現状は業種分類の取得に利用）
- [ ] 業種スコアの Web 設定画面での編集
